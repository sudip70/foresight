...
class ForesightEngine:
    ...
    def __init__(self, settings: Settings, *, strict_validation: bool = True) -> None:
        ...
        self._ticker_lookup_cache: dict[str, tuple[str, int]] = {
            ticker.upper(): (asset_class, index)
            for asset_class, bundle in self.runtime.assets.items()
    ...
    def _ticker_lookup(self) -> dict[str, tuple[str, int]]:
        return self._ticker_lookup_cache

    def _resolve_ticker(self, ticker: str) -> tuple[str, int, str]:
        normalized = ticker.strip().upper()
        lookup = self._ticker_lookup_cache
    ...
    def _public_scenario_daily_return(
        ...
        *,
        ...
        capped = soft_cap_return(blended, lower=lower, upper=upper)
        return capped, {
    ...
    def _ticker_forecast_payload(
        ...
        *,
        ...
        max_drawdown = historical_drawdown(price_series)
    ...
    def ticker_raw_data(self, ticker: str) -> dict:
        """Return price and OHLCV artifact data for a ticker without exposing internal structures."""
        asset_class, index, canonical = self._resolve_ticker(ticker)
        context = self._combined_context["assets"][asset_class]
    ...
    def run_portfolio_simulation(
        ...
        *,
        ...
        if selected_tickers:
            ...
            for forecast in forecasts:
                score = forecast_score(forecast, risk=risk_value)
